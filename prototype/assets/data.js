/* ============================================================
   Redline MVP — content and rules.
   SIT317 Task 8.2HD · Gureijaz Singh Aulakh · s224859639

   Three things live here:
     1. QUESTIONS  — the six-question Readiness Check and its
                     rules-based scoring engine.
     2. E8         — the small-firm subset of the ASD Essential Eight
                     the consultant works through on the day.
     3. CORPUS     — the seeded practice used for the worked example.
                     A real engagement runs the same discovery on the
                     client's own screen; nothing here is uploaded.
   ============================================================ */
(function (global) {
  'use strict';

  /* ================= 1. READINESS CHECK ================= */
  /* score: 0 = worst, 3 = best. "unsure" always scores 0 and is
     recorded as a finding in its own right, because not knowing where
     identity documents are is itself the problem we are looking for. */
  ...
  /* Rules-based scoring. Multi-select questions take their WORST
     selected option, because one uncontrolled location is enough to
     defeat every controlled one. */
  ...
  /* Locations searched during discovery. `items` are what the search
     returns. In a live engagement these come from the practice's own
     Microsoft 365 or Google Workspace search, driven by the principal;
     Redline holds no standing access to the tenancy. */
  var CORPUS = [
    {
  ...
  /* One-click worked example: an engagement that has been through the
     check, the baseline and discovery, with every item decided. Used
     for the walkthrough and so a marker can see a finished Statement
     without clicking through forty-seven files. */
  function seed() {
  ...
  /* ?demo=1 on any page seeds a finished engagement before that page's
     own script runs, so a cold link (or a headless print) shows a real
     document rather than an empty state. Existing work is never
     overwritten — it only fills in what is missing. */
  try {
